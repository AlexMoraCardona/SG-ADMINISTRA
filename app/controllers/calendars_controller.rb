class CalendarsController < ApplicationController
    ...
    def detail
        @calendar = Calendar.find(params[:id])
        @activities = Activity.where("calendar_id = ?", @calendar.id).order(:square_id, :place_id) if @calendar.present? 
    end  
    ...
    def calendar_params
        params.require(:calendar).permit(:day, :month, :year, :name_day, :day_vigente, :adm_calendar_id)
    end 
