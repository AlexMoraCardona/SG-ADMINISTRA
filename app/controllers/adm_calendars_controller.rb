class AdmCalendarsController < ApplicationController
    ...
    def generaractivities
        @adm_calendar = AdmCalendar.find(params[:id]) if params[:id].present?
        if @adm_calendar.generatedactivities == 0
           Activity.crear_actividades(@adm_calendar) 
           @adm_calendar.generatedactivities = 1
           @adm_calendar.save
           redirect_to adm_calendars_path, notice: 'Actividades creadas correctamente'
        else
            redirect_to adm_calendars_path, alert: 'Actividades ya fueron creadas', adm_calendar: :see_other
        end    
    end   
    ...
    def adm_calendar_params
        params.require(:adm_calendar).permit(:year, :bisiesto, :day_initial, :generated, :generatedactivities)
    end 
